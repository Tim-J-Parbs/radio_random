...
def url_works(radio, show_output=False):
    url = radio['url']
    try:
        ...
        if response.status_code == 200:
            ...
            if "audio" in content_type:
                output= f"Stream is active: {url} (Content-Type: {content_type})"
                works = True
            else:
                output = f"Stream may not be audio: {url} (Content-Type: {content_type})"
                works = False
        else:
            output = f"Stream is down: {url} (Status Code: {response.status_code})"
            works = False
    except requests.RequestException as e:
        output = f"Error accessing stream: {url} ({e})"
        works = False
...
def get_favorite_station(favorite_database):
    ...
    if favorite_radios is None or len(favorite_radios) == 0: return None
...
class radio_backend():
    def __init__(self, debug_mode=False):
        ...
        self.client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id='RadioControl', protocol=mqtt.MQTTv5)
    ...
    def on_publish(self, PUBLISH, packet, mid, qos, info):
        return

    def get_radio_from_database(self, database, max_tries):
        """TODO: Maybe check if database is actually empty"""
        works = False
        url_counter = 0

        if database == 'global':
            get = get_global_station
        else:
            get = lambda: get_favorite_station(database)
        this_radio = None
        while not works:
            this_radio = get()
            works = url_works(this_radio)
            url_counter += 1
            if url_counter > max_tries:
                print('This did not work.')
                break
        return this_radio, works
    ...
    def on_message(self, client, userdata, message):
        ...
        try:
            if command == 'request':
                ...
                # stations
                this_radio, works = self.get_radio_from_database(database, self.tryout)
                if not works:
                    print("Sad emoji.")
                    return
